...
# start_df & end_df untuk memfilter all_df -> main_df
...
st.header("Dicoding Collection Dashboard :pencil:")
...
# Menampilkan performa product terbaik dan terburuk
...
# Menampilkan kostumer grafik
st.subheader("Custumers Demographiccs")
...
# Menampilkan visual Kostumer terbaik dengan RFM parameter
...
with col2:
    avg_frequency = round(rfm_df.recency.mean(), 2)
    st.metric("Average Frequency (days)", value=avg_frequency)

with col2:
    avg_frequency = format_currency(rfm_df.monetary.mean(), "AUD", locale="es_CO")
    st.metric("Avrage Monetary", value=avg_frequency)
...
with st.container():
    st.caption("")  # Tambahkan spasi kosong di atas caption
    st.markdown("<div style='text-align: center; opacity: 0.5;'>Copyright (c) Dicoding 2023</div>", unsafe_allow_html=True)